...
//			}
	...
	public void run() throws IOException {
		...
		UnionFind uf = new UnionFind(numConnections * 2);
		HashMap<String, Integer> strToPos = new HashMap<String, Integer>();
		HashMap<Integer, String> posToStr = new HashMap<Integer, String>();
		String[] start = new String[numConnections];
		int next = 0;

		for(int i = 0; i < numConnections; i++) {
			StringTokenizer st = new StringTokenizer(file.readLine());
			start[i] = st.nextToken();
			if(!strToPos.containsKey(start[i])) {
				strToPos.put(start[i], next);
				posToStr.put(next++, start[i]);
			}

			String parent = st.nextToken();
			if(!strToPos.containsKey(parent)) {
				strToPos.put(parent, next);
				posToStr.put(next++, parent);
			}

			uf.union(strToPos.get(start[i]), strToPos.get(parent));
		}

		file.close();

		for(int i = 0; i < numConnections; i++) {
			out.printf("%s %s\n", start[i], posToStr.get(uf.find(strToPos.get(start[i]))));
		}
